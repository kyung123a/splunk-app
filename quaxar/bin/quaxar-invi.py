...
def setup_event_logger(level):
    ...
    logger_filename = os.path.join("/opt/splunk", "var", "log", "splunk", "quaxar-invi.log")
...
def setup_ingest_logger(level):
    ...
    logger.propagate = False
    logger.setLevel(logging.DEBUG)
    logger_handler = logging.handlers.TimedRotatingFileHandler("./export-invi.json", when="midnight", backupCount=10)
